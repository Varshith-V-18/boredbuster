...
SYSTEM_PROMPT = (
    ...
    "exact character match. If there's genuinely no reasonably close "
    "title in the results, follow the tool's own instructions on that — "
    "for theater movies it will give you an unconfirmed fallback search "
    "link to offer instead of a flat dead end, since the live listings "
    "are known to be incomplete for Indian regional releases. Never "
    "silently substitute a different movie without saying so. "
